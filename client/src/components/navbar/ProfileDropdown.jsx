  ...
  return (
    ...
    >
      ...
      >
        {currentUser?.avatar && !avatarFailed ? (
          <img 
            ...
            className="h-13 w-13 rounded-full object-cover ring-2 ring-indigo-500/20"
          />
        ) : (
      ...
      <div className="mt-1.5 space-y-0.5">
        <button
          className={`w-full text-left flex items-center gap-2.5 px-3 py-2 rounded-xl !text-[14px] font-normal transition-all duration-150 ${isDark ? 'text-sky-300 hover:bg-sky-950/30' : 'text-sky-700 hover:bg-sky-50'}`}
          onClick={handleConnectTelegram}
          disabled={isConnectingTelegram}
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21.75 4.5L12 13.5M21.75 4.5L15.75 21l-3.75-7.5L4.5 12l17.25-7.5z"></path></svg>
          {isConnectingTelegram ? "Preparing Telegram link..." : "Connect Telegram"}
        </button>

        <button 
          className={`w-full text-left flex items-center gap-2.5 px-3 py-2 rounded-xl !text-[14px] font-normal transition-all duration-150 ${isDark ? 'text-slate-300 hover:bg-slate-900/60' : 'text-slate-600 hover:bg-slate-50'}`}
